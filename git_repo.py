...
if not access_token:
    raise ValueError("Error: GitHub access token is missing!")

def upload_to_github(file_content, file_name):
    base_url = f"https://api.github.com/repos/Bskn1411/files/contents/hell/{file_name}"
...
def delete_from_github(file_name):
    ...
    if response.status_code == 200:
        ...
        # Delete the file
        response = requests.delete(base_url, headers=headers, data=json.dumps(payload))
        if response.status_code == 200:
...
#                 response = requests.put(base_url, headers=headers, data=payload, timeout=100)
...
# def delete_from_github(file_name):
#     base_url = f"https://api.github.com/repos/Bskn1411/files/contents/{file_name}" 
